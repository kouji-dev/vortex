...
// Canonical auth screen — kept identical to apps/web login.ts. The only
// permitted difference is the "Platform" badge in the brand lockup and the
// auth service. Social sign-in is allowed but only grants access when the
// provider account's email matches a configured platform admin (env).
@Component({
  selector: 'app-login',
  standalone: true,
  imports: [
