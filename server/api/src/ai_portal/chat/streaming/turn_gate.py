from __future__ import annotations

import uuid
from dataclasses import dataclass

from fastapi import HTTPException, status
from sqlalchemy.orm import Session

from ai_portal.auth.model import User as UserModel
from ai_portal.rbac.evaluator import evaluate as rbac_evaluate
from ai_portal.usage.service import check_quota


@dataclass(frozen=True, slots=True)
class GateResult:
    effective_model: str
    allowed_tools: list[str]
    allowed_capabilities: list[str]


def evaluate(
    *,
    session: Session,
    org_id: uuid.UUID,
    user_id: int,
    requested_model: str,
    requested_tools: list[str],
    requested_capabilities: list[str],
) -> GateResult:
    decision = check_quota(session, org_id=org_id, user_id=user_id, api_model_id=requested_model)
    if decision.is_blocked:
        raise HTTPException(
            status.HTTP_429_TOO_MANY_REQUESTS,
            detail={"code": "QUOTA_EXCEEDED", "message": decision.reason},
        )

    from sqlalchemy import select  # noqa: PLC0415
    user = session.scalar(select(UserModel).where(UserModel.id == user_id))
    user_role = user.role if user and user.role else "member"

    class _UserProxy:
        role = user_role

    model_decision = rbac_evaluate(session, user=_UserProxy(), org_id=org_id,
                                   resource_type="model", resource_key=requested_model)
    if not model_decision.allowed:
        raise HTTPException(
            status.HTTP_403_FORBIDDEN,
            detail={"code": "MODEL_FORBIDDEN", "message": model_decision.reason},
        )

    allowed_tools = [
        t for t in requested_tools
        if rbac_evaluate(session, user=_UserProxy(), org_id=org_id,
                         resource_type="tool", resource_key=t).allowed
    ]

    allowed_capabilities = [
        c for c in requested_capabilities
        if rbac_evaluate(session, user=_UserProxy(), org_id=org_id,
                         resource_type="capability", resource_key=c).allowed
    ]

    return GateResult(
        effective_model=requested_model,
        allowed_tools=allowed_tools,
        allowed_capabilities=allowed_capabilities,
    )
