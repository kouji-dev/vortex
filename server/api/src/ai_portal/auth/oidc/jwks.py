...
def _client(uri: str) -> PyJWKClient:
    return _clients.setdefault(uri, PyJWKClient(uri, cache_keys=True))
...
def verify_id_token(token: str, *, jwks_uri: str, issuer: str, audience: str, leeway: int = 30) -> dict:
    ...
    if getattr(signing_key, "key_type", "RSA") != "RSA":
        raise jwt.InvalidKeyError("JWKS signing key is not RSA")
